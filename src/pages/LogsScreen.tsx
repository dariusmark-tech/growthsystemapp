...
export default function LogsScreen() {
  ...
  const { logs, connected, loading } = useArduinoSensors();
      ...
      <div className="flex justify-between items-start mb-1 mt-2">
        <div>
          ...
          <p className="text-text-muted text-xs">
            {logs.length} entries from Arduino · {connected ? 'live' : (loading ? 'connecting…' : 'offline')}
          </p>
        </div>
      ...
      <div className="bg-card rounded-lg border border-border overflow-hidden shadow-[0_1px_4px_0_hsl(152_55%_28%/0.05)]">
        ...
        {filtered.length === 0 ? (
          <div className="p-6 text-center text-text-muted text-xs">
            {connected ? 'No log entries match this filter yet.' : 'Waiting for Arduino sensor data…'}
          </div>
        ) : filtered.map((log, i) => (
